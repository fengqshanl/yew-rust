...
use crate::{errors::errors::MyError, models::drug::{Drug, DrugDetail}, client::client::request};
...
pub async fn get_drug(client: &Client, code: String) -> Result<DrugDetail, MyError> {
    ...
    let res = client
        ...
        .collect::<Vec<DrugDetail>>();
    if res.len() == 0 {
        let detail = request::<DrugDetail>(Method::POST, "http://jumbarcode.market.alicloudapi.com/bar-code/query".to_string(), code, true).await.expect("msg");
        let resu = add_drug(client, detail.clone()).await.expect("msg");
        return Ok(resu)
    }
    Ok(res[0].clone())
}
